...
@section('content')
    ...
    <?php
        ...
        if ($form_todo == 'UPDATE')
        {
            // TABLE FIELDS
            $id      = old('id') ?? $data->id;
            $mission = old('mission') ?? $data->mission;
            $vision  = old('vision') ?? $data->vision;

            $prevent_refresh = 'true';
            $form_action = route('admin.mission-and-vision.update', ['id' => $id]);
            $form_method = 'PUT';
            $form_required = "<code>*</code>";
            $form_button = "
            <button type='button'
                target-module='". route('admin.mission-and-vision') ."'
                ...
                <div class="row g-3 align-items-center">
                    <div class="col-9">
                        ...
                        <ol class="breadcrumb bg-transparent mb-0">
                            ...
                            <li class="breadcrumb-item"><a class="btnCancel" href="#" target-module="{{ route('admin.mission-and-vision') }}">Mission and Vision</a></li>
                ...
                <div class="row">
                    <div class="col-12">
                        ...
                        <form action="{{ $form_action }}"
                            method="POST"
                            id="formCompanyMissionandVision"
                            form-todo="{{ $form_todo }}"
                            validated="false"
                            ...
                            <div class="card">
                                <div class="card-body py-3">
                                    <div class="form-group row my-2">
                                        <label for="mission" class="col-sm-2 col-form-label">
                                            Mission <?= $form_required ?>
                                        </label>
                                        <div class="col-sm-10">
                                            <textarea name="mission" id="mission" rows="8" style="resize: none;"
                                                    class="form-control" autocomplete="off" required>{{ $mission }}</textarea>
                                        </div>
                                    </div>
                                    <div class="form-group row my-2">
                                        <label for="vision" class="col-sm-2 col-form-label">
                                            Vision <?= $form_required ?>
                                        </label>
                                        <div class="col-sm-10">
                                            <textarea name="vision" id="vision" rows="8" style="resize: none;"
                                                    class="form-control" autocomplete="off" required>{{ $vision }}</textarea>
                                        </div>
                                    </div>
                                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            // ----- SUBMIT FORM -----
            $(document).on('submit', '#formCompanyMissionandVision', function(e) {
                    ...
                    let confirmation = $.confirm({
                        ...
                        buttons: {
                            ...
                            yes: {
                                ...
                                action: function() {
                                    preventRefresh(false);
                                    $('#formCompanyMissionandVision').attr('validated', 'true').submit();
