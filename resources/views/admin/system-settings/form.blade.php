...
@section('content')
    ...
    <?php
        ...
        if (!isset($form_todo) || $form_todo == 'CREATE')
        {
            ...
            $form_action = route('admin.system-settings.save');
            $form_method = 'POST';
            $form_required = "<code>*</code>";
            $form_button = "
            <button type='button'
                target-module='". route('admin.system-settings') ."'
        ...
        else if ($form_todo == 'READ')
        {
            ...
            <div class='d-flex justify-content-end'>
                ...
                <a href=". route('admin.system-settings.edit', ['id' => $id]) ."
        ...
        else if ($form_todo == 'UPDATE')
        {
            // TABLE FIELDS
            $id          = old('id') ?? $data->id;
            $key         = old('key') ?? $data->key;
            $value       = old('value') ?? $data->value;
            $description = old('description') ?? $data->description;

            $prevent_refresh = 'true';
            $form_action = route('admin.system-settings.update', ['id' => $id]);
            $form_method = 'PUT';
            $form_required = "<code>*</code>";
            $form_button = "
            <button type='button'
                ...
                <div class="row g-3 align-items-center">
                    <div class="col-9">
                        ...
                        <ol class="breadcrumb bg-transparent mb-0">
                            <li class="breadcrumb-item"><a class="text-secondary" href="#">Administrator</a></li>
                            <li class="breadcrumb-item"><a class="btnCancel" href="#" target-module="{{ route('admin.system-settings') }}">System Settings</a></li>
            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-12">
                            ...
                            <div class="card">
                                <div class="card-body py-3">
                                    ...
                                    <div class="form-group row my-2">
                                        ...
                                        <div class="col-sm-10">
                                            @if ($form_todo == 'READ')
                                                <textarea name="value" id="value" rows="8" style="resize: none;"
                                                    class="form-control" disabled>{{ $value ?? '-' }}</textarea>
                                            @else
                                                <textarea name="value" id="value" rows="8" style="resize: none;"
                                                    class="form-control" autocomplete="off" required>{{ $value }}</textarea>
                                            @endif
                                        </div>
                                    </div>
                                    <div class="form-group row my-2">
                        ...
                        @if ($form_todo != 'CREATE')
                            <!-- ----- FORM DELETE ----- -->
                            <form action='{{ route('admin.system-settings.delete', ['id' => $id]) }}' method='POST' id='formDelete'>
                                @csrf 
                                @method('DELETE')
                            </form>
                            <!-- ----- END FORM DELETE ----- -->
