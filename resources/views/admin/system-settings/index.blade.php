...
@section('content')
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="w-100 text-end mb-3">
                            <a href="{{ route('admin.system-settings.create') }}" class="btn btn-outline-primary px-2 py-1 w-sm-100">
                                <i class="bi bi-plus-lg"></i> New
                            </a>
                        </div>

                        <table class="table table-striped table-hover" id="tableSystemSettings">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>Key</th>
                                ...
                                @if (isset($data) && !empty($data))
                                    @foreach ($data as $index => $dt)
                                        <tr>
                                            <td>{{ $index + 1 }}</td>
                                            <td>
                                                <a href="{{ route('admin.system-settings.view', ['id' => $dt->id]) }}" 
                                                ...
                                                <ul class="dropdown-menu shadow border-0 p-2">
                                                    <li>
                                                        <a class="dropdown-item" 
                                                            href="{{ route('admin.system-settings.edit', ['id' => $dt->id]) }}">
                                                    ...
                                                    <li>
                                                        ...
                                                        <!-- ----- FORM DELETE ----- -->
                                                        <form action="{{ route('admin.system-settings.delete', ['id' => $dt->id]) }}" 
                                                            method="POST" id="formDelete{{ $index }}">
                                                            @csrf
                                                            @method('DELETE')
                                                        </form>
                                                        <!-- ----- END FORM DELETE ----- -->
    ...
    <script>
        $(document).ready(function() {
            // ----- DATATABLES -----
            let tableSystemSettings = $('#tableSystemSettings')
                .css({ "min-width": "99%" })
                .removeAttr("width")
                .DataTable({
                    scrollX: true,
